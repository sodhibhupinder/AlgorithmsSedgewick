...
/**
 * 			@author baddie Tieu owns a pizza restaurant and he manages it in his own way.
 ...
 *         Constraints
 * 
 *         1 ≤ N ≤ 105 0 ≤ Ti ≤ 109 1 ≤ Li ≤ 109 Note
 * 
 ...
 *         8 Explanation #01
 * 
 *         Let's call the person ordering at time = 0 as A, time = 1 as B and
 *         time = 2 as C. By delivering pizza for A, C and B we get the minimum
 *         average wait time to be
 * 
 *         (3 + 6 + 16)/3 = 25/3 = 8.33 the integer part is 8 and hence the
 *         answer.
 */
	...
	public static void main(String[] args) {
		...
		while (!costumers.isEmpty() || !waiting.isEmpty()) {
			...
			while (!costumers.isEmpty() && costumers.peek().arrivalTime <= endTime) {
				waiting.add(costumers.remove()); // Add costumers to the waiting
													// list
			}
		}
