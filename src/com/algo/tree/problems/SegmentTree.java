...
public class SegmentTree {
	 ...
	 * 
	 * st --> Pointer to segment tree si --> Index of current node in the
	 ...
	 * node, i.e., st[si] qs & qe --> Starting and ending indexes of query range
	 */
	 ...
	 * their range. The following are parameters st, si, ss and se are same as
	 * getSumUtil() i --> index of the element to be updated. This index is in
	 * input array. diff --> Value to be added to all nodes which have i in
	 * range
	 */
	...
	public static void main(String args[]) {
		...
		for(int i=0;i<Q;i++)
		{
			...
			int u = scan.nextInt();
			int x = scan.nextInt();
			tree.updateValue(arr, n, u, x);
			}
			else if(operation==2)
			{
				int u = scan.nextInt();
				int v = scan.nextInt();
				System.out.println(tree.getSum(n,u,v-1));
			}
		}
	}
}
